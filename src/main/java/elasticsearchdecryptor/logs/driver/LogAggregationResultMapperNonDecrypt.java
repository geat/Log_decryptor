...
import elasticsearchdecryptor.logs.model.LogInfoNonDecrypt;
...
class LogAggregationResultMapperNonDecrypt {

  private static final TypeReference<LogInfoNonDecrypt> DRIVER_DATA_TYPE_REFERENCE = new TypeReference<>() {
  };

  private final ObjectMapper objectMapper;

  LogsDataNonDecrypt map(SearchResponse searchResponse) {
    Aggregations aggregations = searchResponse.getAggregations();
    return LogsDataNonDecrypt.builder()
  ...
  private List<LogInfoNonDecrypt> getLogsList(SearchResponse searchResponse) {
    return stream(searchResponse.getHits().getHits())
        ...
        .sorted(Comparator.comparing(LogInfoNonDecrypt::getApiId))
        .collect(toList());
